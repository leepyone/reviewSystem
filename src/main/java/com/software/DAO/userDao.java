 ...
 */
    ...
    User Login(String user_number,String user_password);//账号密码登录

    //根据身份证查询用户
    @Select("select * from user where `user`.user_identifyID=#{userIdentifyID}")
    User getUserByIDCard(String userIdentifyID);//根据身份证查询用户

    //根据电话号码查询用户
    @Select("select * from user where `user`.user_phone=#{user_phone}")
    User getUserByPhone(String user_phone);//根据电话号码查询用户
    ...
    int insertUser(User user);//插入用户信息,注册


    //修改用户的密码
    @Update("update `user` set `user`.user_password=#{userPassword}  where `user`.user_ID=#{userId}")
    boolean changeUserPassword(String userPassword,int userId);//修改用户的密码

    //修改用户的类别
    @Update("update `user` set `user`.user_type=#{userType}  where `user`.user_ID=#{userId}")
    boolean changeUserType(int userType,int userId);//修改用户的类别

    //修改用户的公司id
    @Update("update `user` set `user`.user_corpID=#{CorpId}  where `user`.user_ID=#{userId}")
    boolean changeUserCorpId(int CorpId,int userId);//修改用户的公司id
...
//    @Options(useGeneratedKeys = true, keyProperty = "userId", keyColumn = "user_ID")
    int insertUserDetail(UserDetails userDetails);//插入一条用户详细信息记录
    ...
    boolean changeUserDetail(UserDetails userDetails);//修改用户的详细信息
